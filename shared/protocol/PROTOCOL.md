...
- `sessionId` (string, опционально) - уникальный идентификатор сессии для поддержки постоянных соединений (HTTPS, WebSocket и т.д.)
- `isFirstInSession` (boolean, опционально) - является ли это первым запросом в сессии
- `keepSessionAlive` (boolean, опционально) - сохранять ли TCP соединение после обработки ответа (для HTTPS, WebSocket и т.д.)
...
Для поддержки протоколов, требующих постоянных соединений (HTTPS, WebSocket и т.д.), используется система управления сессиями.

### Концепция сессий

- **Сессия** - это постоянное TCP соединение между LISTENER и целевым сервером (GOAL)
- Для порта 443 (HTTPS) автоматически создаются и переиспользуются сессии
- Для других портов каждый запрос создает новое соединение (одноразовая "сессия")

### Работа с сессиями

1. **Первый запрос в сессии** (`isFirstInSession: true`):
   - CALLER создает новую сессию с уникальным `sessionId`
   - LISTENER создает новое TCP соединение с целевым сервером
   - Соединение сохраняется для последующих запросов

2. **Последующие запросы в сессии** (`isFirstInSession: false`):
   - CALLER переиспользует существующий `sessionId`
   - LISTENER переиспользует существующее TCP соединение
   - TLS state сохраняется между раундами handshake

3. **Закрытие сессии**:
   - Если `keepSessionAlive: false`, соединение закрывается после ответа
   - Если `keepSessionAlive: true`, соединение сохраняется для следующих запросов
   - Неактивные сессии автоматически закрываются через 60 секунд неактивности

## Особенности реализации

### Генерация requestId и sessionId
