...
Deno.test("Monitor - не обрабатывает уже известные файлы", async () => {
  ...
  // Проверяем, что файл, найденный при инициализации, обрабатывается один раз
  // (теперь файлы, найденные при инициализации, обрабатываются)
  assertEquals(discoveredFiles.length, 1);
});
