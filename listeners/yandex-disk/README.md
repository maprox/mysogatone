# LISTENER для Яндекс Диск

Сервер в интернете, который мониторит Яндекс Диск и обрабатывает запросы на подключение.

## Описание

LISTENER подключается к Яндекс Диск аккаунту пользователя через API и:
- Мониторит изменения в определенной папке на Яндекс Диске
- Обнаруживает запросы на подключение (папки/файлы)
- Подключается к целевому серверу (GOAL)
- Записывает ответы от GOAL в Яндекс Диск папку через API

## Требования

- Deno runtime
- OAuth токен для доступа к Яндекс Диск API

## Настройка

### Быстрый старт (если есть Client ID)

1. Откройте в браузере (замените `YOUR_CLIENT_ID`):
   ```
   https://oauth.yandex.ru/authorize?response_type=token&client_id=YOUR_CLIENT_ID
   ```
2. Разрешите доступ
3. Скопируйте токен из URL: `access_token=ВАШ_ТОКЕН`
4. Проверьте токен:
   ```bash
   deno task health ВАШ_ТОКЕН
   ```

**Готово!** Теперь можно тестировать.

📖 Подробная инструкция: [QUICK_START.md](QUICK_START.md) или [GETTING_TOKEN.md](GETTING_TOKEN.md)

## Запуск

```bash
deno task start
```

Или напрямую:

```bash
deno run --allow-net --allow-read --allow-write src/main.ts
```

## Линтинг и проверка кода

Проверка кода линтером:

```bash
deno task lint
```

Автоматическое исправление проблем:

```bash
deno task lint:fix
```

Проверка типов и линтинг:

```bash
deno task check
```

## Тестирование

Запуск всех тестов:

```bash
deno task test
```

Запуск тестов в режиме watch (автоматический перезапуск при изменениях):

```bash
deno task test:watch
```

Запуск конкретного тестового файла:

```bash
deno test --allow-net --allow-read --allow-write tests/utils_test.ts
```

### Структура тестов

Тесты находятся в папке `tests/`:
- `utils_test.ts` - тесты для утилит (нормализация путей, построение URL, задержки)
- `errors_test.ts` - тесты для классов ошибок
- `file-operations_test.ts` - тесты для операций с файлами
- `http-client_test.ts` - тесты для HTTP клиента и retry логики
- `yandex-disk-provider_test.ts` - тесты для основного провайдера
- `test_runner.ts` - главный файл для запуска всех тестов

## Проверка работоспособности

### Быстрая проверка настройки (без реального API)

Проверяет импорты, типы и базовую функциональность:

```bash
deno task verify
```

Или напрямую:

```bash
deno run --allow-net --allow-read --allow-write scripts/verify-setup.ts
```

### Полная проверка с реальным API

Проверяет работоспособность с реальным Яндекс Диск API. Требует OAuth токен.

```bash
# Через аргумент командной строки
deno task health [YOUR_TOKEN]

# Или через переменную окружения
export YANDEX_DISK_TOKEN=your_token
deno task health
```

Или напрямую:

```bash
deno run --allow-net --allow-read --allow-env scripts/check-health.ts [YOUR_TOKEN]
```

Скрипт проверяет:
- ✅ Создание провайдера
- ✅ Получение списка файлов
- ✅ Загрузку файла
- ✅ Скачивание файла
- ✅ Удаление файла

### Комплексная проверка

Запуск всех проверок последовательно:

```bash
# 1. Проверка настройки
deno task verify

# 2. Линтинг
deno task lint

# 3. Проверка типов
deno task check

# 4. Запуск тестов
deno task test

# 5. Проверка с реальным API (опционально)
deno task health [YOUR_TOKEN]
```

### Отладка проблем

Если что-то не работает:

1. **Проверьте настройку проекта:**
   ```bash
   deno task verify
   ```

2. **Проверьте линтинг:**
   ```bash
   deno task lint
   ```

3. **Проверьте типы:**
   ```bash
   deno task check
   ```

4. **Запустите тесты:**
   ```bash
   deno task test
   ```

5. **Проверьте с реальным API (если есть токен):**
   ```bash
   deno task health [YOUR_TOKEN]
   ```

## Структура

- `main.ts` - Точка входа приложения
- `storage-provider/` - Модуль StorageProvider для Яндекс Диск API
  - `types.ts` - Типы и интерфейсы
  - `errors.ts` - Классы ошибок
  - `utils.ts` - Утилиты
  - `http-client.ts` - HTTP клиент с retry механизмом
  - `file-operations.ts` - Операции с файлами
  - `yandex-disk-provider.ts` - Основной класс провайдера
  - `index.ts` - Публичный API модуля
- `monitor.ts` - Polling механизм для мониторинга изменений
- `connection-handler.ts` - Обработка подключений к целевым серверам

## Протокол обмена

Формат запросов и ответов описан в [shared/protocol/PROTOCOL.md](../../shared/protocol/PROTOCOL.md).

Протокол использует файлы в облачном хранилище:
- `requests/{requestId}.req` - метаданные запроса (JSON)
- `requests/{requestId}.data` - данные для отправки на GOAL (binary)
- `responses/{requestId}.resp` - ответ от GOAL (binary)
- `responses/{requestId}.error` - ошибка при обработке (JSON)

