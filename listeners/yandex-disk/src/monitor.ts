import type { FileInfo, StorageProvider } from "@src/storage-provider/index.ts";
...
export class Monitor {
  ...
  async start(
    ...
    console.log(
      `[Monitor] Инициализация: получение списка файлов из ${this.watchFolder}...`,
    );
    const initialFiles = await this.storageProvider.listFiles(this.watchFolder);
    console.log(
      `[Monitor] Найдено ${initialFiles.length} файлов при инициализации`,
    );
    ...
    for (const file of initialFiles) {
      ...
      const normalizedPath = file.path.startsWith("disk:")
        ...
        : file.path;
      console.log(
        `[Monitor] Добавлен в knownFiles: ${normalizedPath} (оригинальный: ${file.path})`,
      );
      ...
      if (normalizedPath.endsWith(".req")) {
        console.log(
          `[Monitor] Обработка файла, найденного при инициализации: ${normalizedPath}`,
        );
        await onNewRequest({ ...file, path: normalizedPath });
      }
    }
    console.log(
      `[Monitor] Инициализация завершена. Известных файлов: ${this.knownFiles.size}`,
    );
  ...
  private async checkForNewFiles(
    ...
    try {
      ...
      console.log(
        `[Monitor] Проверка папки ${this.watchFolder}: найдено ${files.length} файлов`,
      );
      ...
      for (const file of files) {
        ...
        if (!this.knownFiles.has(normalizedPath)) {
          ...
          console.log(
            `[Monitor] Обнаружен новый запрос: ${normalizedPath} (оригинальный путь: ${file.path})`,
          );
          this.knownFiles.add(normalizedPath);
      ...
      console.error(
        `[Monitor] Ошибка при проверке файлов в ${this.watchFolder}:`,
        error,
      );
      throw error;
    }
